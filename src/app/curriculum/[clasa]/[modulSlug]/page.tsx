    ...
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3">
        ...
        <Link
          ...
          className={`flex items-center justify-center gap-2 rounded-2xl border p-3 text-center text-sm font-bold transition ${
            esteGratuit
              ? "border-success/20 bg-success/5 hover:bg-success/10 text-success"
              : "border-brand-border bg-brand-light/45 hover:bg-brand-light text-brand-dark"
          }`}
        >
          <span>{esteGratuit ? "📝" : "🔒"}</span> Exerciții practice
          <span className={`rounded px-1.5 py-0.5 text-[9px] font-bold ml-1 ${
            esteGratuit ? "bg-success/20 text-success" : "bg-brand/20 text-brand-dark"
          }`}>
            {esteGratuit ? "Gratis" : "Premium"}
          </span>
        </Link>
      </div>
